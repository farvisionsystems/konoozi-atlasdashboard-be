defmodule AtlasWeb.UserAuth do
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_atlas_web_user_remember_me"
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      AtlasWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
    ...
    case get_req_header(conn, "authorization") do
      ...
      [token] ->
        if String.contains?(token, " ") do
          ...
          if token_exists? do
            user = Accounts.get_user_by_session_token(binary_token)
  ...
  the current_user:
      ...
      defmodule AtlasWeb.PageLive do
        ...
        on_mount {AtlasWeb.UserAuth, :mount_current_user}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{AtlasWeb.UserAuth, :ensure_authenticated}] do
  ...
  def require_logged_in_user(conn, _opts) do
    if conn.assigns[:current_user] do
      conn
    else
      conn
